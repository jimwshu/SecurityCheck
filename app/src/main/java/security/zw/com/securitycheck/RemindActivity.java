...
import security.zw.com.securitycheck.adapter.RemindAdapter;
...
public class RemindActivity extends BaseSystemBarTintActivity {
    ...
    */
    ...
    protected RemindAdapter mAdapter;
    ...
    private void initWidget() {
        ...
        mAdapter = new RemindAdapter(data, this, 3);
        ...
        mAdapter.setOnClick(new RemindAdapter.onClick() {
            @Override
    ...
    private void loadData() {
        ...
        mCall = addCheck.getRemind(requestBody);
    ...
    private void post(ProjectInfo projectInfo) {
        if (isLoading) {
            return;
        }
        isLoading = true;
        ...
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("userId", SecurityApplication.mUser.id);
        jsonObject.addProperty("id", projectInfo.id);
        jsonObject.addProperty("recordControl", 1);


        String s = jsonObject.toString();
        RequestBody requestBody = RequestBody.create(okhttp3.MediaType.parse("application/json; charset=utf-8"), s);

        mCall = addCheck.changeRemind(requestBody);
